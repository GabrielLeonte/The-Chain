...
class Blockchain {
  ...
  init() {
    ...
    this.chain.serialize(() => {
      ...
      this.chain.run("CREATE TABLE IF NOT EXISTS blocks (block INTEGER PRIMARY KEY,previousHash TEXT, hash TEXT, timestamp TEXT, header TEXT)", err => {
      ...
      this.chain.get(`SELECT * FROM blocks WHERE block = 1`, (err, data) => {
        ...
        if (!data)
          this.chain.run(
            `INSERT INTO blocks (previousHash, hash, timestamp, header) VALUES ("${this.block.previousHash}","${this.block.hash}","${this.block.timestamp}", "${this.block.header}")`,
            err => {
  ...
  createNewBlock() {
    let previousHash = this.block.hash;
    let data = this.data;
    return (this.block = {
      hash: hash(data),
