...
class Blockchain {
  ...
  getLast100() {
    try {
      return this.chain.prepare("SELECT * FROM blocks ORDER BY block").all();
    } catch (err) {
  ...
  push(data) {
    if (typeof data !== "object") return console.warn("push method: The data pushed must be an object!");
    this.data.push(data);
  }
}
